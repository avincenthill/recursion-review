...
var parseJSON = function(originalString) {
  // I: JSON String
  // O: JSON Object
  // C: Must follow JSON grammar
  // E: Incorrectly formatted strings

  let at, ch;
  at = 0;
  ch = originalString.charAt(at);

  var error = function(message) {
    // throw error for bad syntax
  ...
  let next = () => {
    at++;
    //ch is the current character in the string iteration
    ch = originalString.charAt(at);
    console.log('currIndex: ', at, 'currChar: ', ch);
    return ch;
  };
  ...
  // handle value
  let valueF = () => {
    //case select on what the character is
    switch (ch) {
    case '{':
      return objectF();
    case '[':
